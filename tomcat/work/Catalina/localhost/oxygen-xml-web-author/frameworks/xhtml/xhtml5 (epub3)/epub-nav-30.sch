<?xml version="1.0" encoding="UTF-8"?><schema xmlns="http://purl.oclc.org/dsdl/schematron" queryBinding="xslt2">
    ...
    <pattern id="landmarks">
        <rule context="html:nav[tokenize(@epub:type,'\s+')='landmarks']//html:ol//html:a">
            ...
            -->
            <assert test="                 empty(ancestor::html:nav//html:ol//html:a[                     not(. is $current) and                     tokenize(lower-case(@epub:type),'\s+') = $current_type_normalized and                     normalize-space(lower-case(@href)) = $current_href_normalized                     ])">Another landmark was found with the same epub:type and same reference to "<value-of select="$current_href_normalized"/>"</assert>
        </rule>
    </pattern>
    ...
    <pattern id="req-heading">
        <rule context="html:nav[@epub:type][not(tokenize(@epub:type,'\s+') = ('toc','page-list','landmarks'))]">
