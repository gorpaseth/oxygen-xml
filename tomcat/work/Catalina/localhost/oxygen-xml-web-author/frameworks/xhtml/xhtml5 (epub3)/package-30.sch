<?xml version="1.0" encoding="UTF-8"?><schema xmlns="http://purl.oclc.org/dsdl/schematron" queryBinding="xslt2">
    ...
    <pattern id="opf.dcterms.modified">
        <rule context="opf:package/opf:metadata">
            <assert test="count(opf:meta[normalize-space(@property)='dcterms:modified' and not(@refines)]) = 1">package dcterms:modified meta element must occur exactly once</assert>
        </rule>
    </pattern>
    ...
    <pattern id="opf.refines.by-fragment">
        <rule context="*[not(ancestor::opf:collection)][@refines]">
            ...
            <report test="$item">WARNING: @refines should instead refer to "<value-of select="@refines"/>" using a fragment identifier pointing to its manifest item ("#<value-of select="$item/@id"/>")
            </report>
        </rule>
    </pattern>
    ...
    <pattern id="opf.meta.authority">
        <rule context="opf:meta[normalize-space(@property)='authority']">
            <assert test="exists(../dc:subject[concat('#',normalize-space(@id)) = normalize-space(current()/@refines)])">Property "authority" must refine a "subject" property.</assert>
    ...
    <pattern id="opf.meta.belongs-to-collection">
    	<rule context="opf:meta[normalize-space(@property)='belongs-to-collection']">
            <assert test="empty(@refines) or exists(../opf:meta[normalize-space(@id)=substring(normalize-space(current()/@refines),2)][normalize-space(@property)='belongs-to-collection'])">Property "belongs-to-collection" can only refine other "belongs-to-collection"
                properties.</assert>
        </rule>
    </pattern>
    ...
    <pattern id="opf.meta.collection-type">
        <rule context="opf:meta[normalize-space(@property)='collection-type']">
            <assert test="exists(../opf:meta[normalize-space(@id)=substring(normalize-space(current()/@refines),2)][normalize-space(@property)='belongs-to-collection'])">Property "collection-type" must refine a "belongs-to-collection" property.</assert>
            <report test="exists(preceding-sibling::opf:meta[normalize-space(@property) = normalize-space(current()/@property)][normalize-space(@refines) = normalize-space(current()/@refines)])">Property "collection-type" cannot be declared more than once to refine the same "belongs-to-collection" expression.</report>
        </rule>
    </pattern>
    
    <pattern id="opf.meta.display-seq">
        <rule context="opf:meta[normalize-space(@property)='display-seq']">
            <report test="exists(preceding-sibling::opf:meta[normalize-space(@property) = normalize-space(current()/@property)][normalize-space(@refines) = normalize-space(current()/@refines)])">Property "display-seq" cannot be declared more than once to refine the same expression.</report>
        </rule>
    </pattern>
    
    <pattern id="opf.meta.file-as">
        <rule context="opf:meta[normalize-space(@property)='file-as']">
            <report test="exists(preceding-sibling::opf:meta[normalize-space(@property) = normalize-space(current()/@property)][normalize-space(@refines) = normalize-space(current()/@refines)])">Property "file-as" cannot be declared more than once to refine the same expression.</report>
        </rule>
    </pattern>
    
    <pattern id="opf.meta.group-position">
        <rule context="opf:meta[normalize-space(@property)='group-position']">
            <report test="exists(preceding-sibling::opf:meta[normalize-space(@property) = normalize-space(current()/@property)][normalize-space(@refines) = normalize-space(current()/@refines)])">Property "group-position" cannot be declared more than once to refine the same expression.</report>
        </rule>
    </pattern>
    
    <pattern id="opf.meta.identifier-type">
        <rule context="opf:meta[normalize-space(@property)='identifier-type']">
            <assert test="exists(../(dc:identifier|dc:source)[concat('#',normalize-space(@id)) = normalize-space(current()/@refines)])">Property "identifier-type" must refine an "identifier" or "source" property.</assert>
            <report test="exists(preceding-sibling::opf:meta[normalize-space(@property) = normalize-space(current()/@property)][normalize-space(@refines) = normalize-space(current()/@refines)])">Property "identifier-type" cannot be declared more than once to refine the same expression.</report>
        </rule>
    </pattern>
    
    <pattern id="opf.meta.role">
        <rule context="opf:meta[normalize-space(@property)='role']">
            <assert test="exists(../(dc:creator|dc:contributor|dc:publisher)[concat('#',normalize-space(@id)) = normalize-space(current()/@refines)])">Property "role" must refine a "creator", "contributor", or "publisher" property.</assert>
        </rule>
    </pattern>
    ...
    <pattern id="opf.meta.source-of">
        <rule context="opf:meta[normalize-space(@property)='source-of']">
            ...
            <assert test="exists(@refines) and exists(../dc:source[normalize-space(@id)=substring(normalize-space(current()/@refines),2)])">The "source-of" property must refine a "source" property.</assert>
            <report test="exists(preceding-sibling::opf:meta[normalize-space(@property) = normalize-space(current()/@property)][normalize-space(@refines) = normalize-space(current()/@refines)])">Property "source-of" cannot be declared more than once to refine the same "source" expression.</report>
        </rule>
    </pattern>
    
    <pattern id="opf.meta.term">
        <rule context="opf:meta[normalize-space(@property)='term']">
            <assert test="exists(../dc:subject[concat('#',normalize-space(@id)) = normalize-space(current()/@refines)])">Property "term" must refine a "subject" property.</assert>
            <!-- Cardinality is checked in opf.dc.subject.authority-term -->
        </rule>
    </pattern>
    ...
    <pattern id="opf.meta.title-type">
        <rule context="opf:meta[normalize-space(@property)='title-type']">
            <assert test="exists(../dc:title[concat('#',normalize-space(@id)) = normalize-space(current()/@refines)])">Property "title-type" must refine a "title" property.</assert>
            <report test="exists(preceding-sibling::opf:meta[normalize-space(@property) = normalize-space(current()/@property)][normalize-space(@refines) = normalize-space(current()/@refines)])">Property "title-type" cannot be declared more than once to refine the same "title" expression.</report>
        </rule>
    </pattern>
    ...
    <pattern id="opf.nav.prop">
        <rule context="opf:manifest">
            <let name="item" value="//opf:manifest/opf:item[@properties and tokenize(@properties,'\s+') = 'nav']"/>
    ...
    <pattern id="opf.nav.type">
        <rule context="opf:manifest/opf:item[@properties and tokenize(@properties,'\s+') = 'nav']">
    ...
    <pattern id="opf.cover-image">
        <rule context="opf:manifest">
            <let name="item" value="//opf:manifest/opf:item[@properties and tokenize(@properties,'\s+')='cover-image']"/>
    ...
    <pattern id="opf.collection.refines-restriction">
        <rule context="opf:collection/opf:metadata/*[@refines]">
        	<let name="refines-target-id" value="substring(normalize-space(@refines), 2)"/>
            <assert test="starts-with(normalize-space(@refines),'#') and ancestor::opf:collection[not(ancestor::opf:collection)]//*[normalize-space(@id)=$refines-target-id]"> @refines must point to an element within the current collection </assert>
        </rule>
    </pattern>
    ...
    <pattern id="opf_guideReferenceUnique">
        ...
        <rule context="opf:reference">
            ...
            <assert test="                 count(//opf:reference[                     normalize-space(lower-case(@type)) = $current_type_normalized and                     normalize-space(lower-case(@href)) = $current_href_normalized                 ]) le 1">WARNING: Duplicate "reference" elements with the same "type" and "href" attributes</assert>
        </rule>
    </pattern>
	...
	<pattern id="opf.duration.metadata.item">
		<rule context="opf:meta[normalize-space(@property)='media:duration']">
			<assert test="matches(normalize-space(),'^(([0-9]+:[0-5][0-9]:[0-5][0-9](\.[0-9]+)?)|((\s*)[0-5][0-9]:[0-5][0-9](\.[0-9]+)?(\s*))|((\s*)[0-9]+(\.[0-9]+)?(h|min|s|ms)?(\s*)))$')">The value of the media:duration property must be a valid SMIL3 clock value</assert>
		</rule>
	</pattern>
	...
	<pattern id="opf.media.overlay">
		<rule context="opf:item[@media-overlay]">
            ...
            <assert test="$media-type='application/xhtml+xml' or $media-type='image/svg+xml'">The media-overlay attribute is only allowed on XHTML and SVG content documents.</assert>
		</rule>
	</pattern>
